-- ══════════════════════════════════════════════════════════════
-- FlowCanvas — Schema SQL para Supabase
...
-- NOTA ACADÉMICA: estas políticas permiten acceso público para la demo.
-- En producción real, se restringirían por usuario autenticado:
--   USING (auth.uid() = user_id)
-- y se agregaría una columna user_id a cada tabla.
...
-- Política de acceso público (demo académica)
CREATE POLICY "public_all_etapas"
  ON etapas FOR ALL USING (true) WITH CHECK (true);

CREATE POLICY "public_all_clientes"
  ON clientes FOR ALL USING (true) WITH CHECK (true);

CREATE POLICY "public_all_trabajos"
  ON trabajos FOR ALL USING (true) WITH CHECK (true);

CREATE POLICY "public_all_historial"
  ON historial_etapas FOR ALL USING (true) WITH CHECK (true);
...
-- Trabajos de prueba (referenciamos las etapas y clientes por nombre para evitar hardcodear UUIDs)
DO $$
DECLARE
  e_pedido   UUID;
  e_diseno   UUID;
...
BEGIN
  ...
  INSERT INTO trabajos (nombre, cliente_id, etapa_id, prioridad, descripcion) VALUES
    ('1000 Menús plastificados',     c_rest, e_pedido, 'alta',  'Menús A4 plastificados, 2 caras, full color.')
  RETURNING id INTO t1;
  INSERT INTO historial_etapas (trabajo_id, etapa_id) VALUES (t1, e_pedido);

  INSERT INTO trabajos (nombre, cliente_id, etapa_id, prioridad, descripcion) VALUES
    ('Tags para ropa — colección verano', c_bout, e_diseno, 'media', 'Tags 5x5cm, cartón 350g, logo en dorado.')
  RETURNING id INTO t2;
  INSERT INTO historial_etapas (trabajo_id, etapa_id) VALUES (t2, e_diseno);

  INSERT INTO trabajos (nombre, cliente_id, etapa_id, prioridad, descripcion) VALUES
    ('Banner 3x1m apertura gimnasio',    c_gym,  e_prod,   'alta',  'Banner PVC 13oz, full color, con ojales cada 50cm.')
  RETURNING id INTO t3;
  INSERT INTO historial_etapas (trabajo_id, etapa_id) VALUES (t3, e_prod);

  INSERT INTO trabajos (nombre, cliente_id, etapa_id, prioridad, descripcion) VALUES
    ('Menú digital — versión verano',    c_cafe, e_diseno, 'baja',  'PDF interactivo 12 páginas, paleta café y crema.');

  INSERT INTO trabajos (nombre, cliente_id, etapa_id, prioridad, descripcion) VALUES
    ('Cajas personalizadas x500',        c_pan,  e_entrega,'media', 'Cajas troqueladas 20x15x10cm, barniz UV selectivo.');

  INSERT INTO trabajos (nombre, cliente_id, etapa_id, prioridad, descripcion) VALUES
    ('Volantes promocionales x2000',     c_gym,  e_pedido, 'media', 'Volantes A5, 90g, full color ambas caras.');
END $$;
